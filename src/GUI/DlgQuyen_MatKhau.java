...
public class DlgQuyen_MatKhau extends javax.swing.JDialog {
    ...
    private ArrayList<QuyenDTO> listQuyen = new ArrayList<>();
...
//        btnLuuQuyen.setEnabled(false);
    ...
    txtMatKhau_TenDangNhap.setText("");

    cmbQuyen.removeAllItems();

    listQuyen = new ArrayList<>(quyenBUS.getAllQuyen());

    for (QuyenDTO q : listQuyen) {
        cmbQuyen.addItem(q.getTenQuyen());
    }

    String maQuyen = taiKhoanBUS.getQuyenTheoMa(maNV);

    for (int i = 0; i < listQuyen.size(); i++) {
        if (listQuyen.get(i).getMaQuyen().equals(maQuyen)) {
    ...
    private void initComponents() {
        ...
        txtMatKhau_TenDangNhap.setEditable(true);
    ...
    private void btnCapMatKhauActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnCapMatKhauActionPerformed
        if(taiKhoanBUS.datLaiMatKhau(maNV, txtMatKhau_TenDangNhap.getText())){
        new MyDialog("Đặt lại mật khẩu thành công!", MyDialog.SUCCESS_DIALOG);
    }else{
        new MyDialog("Đặt lại mật khẩu thất bại!", MyDialog.ERROR_DIALOG);
    }
    ...
    if(taiKhoanBUS.datLaiQuyen(maNV, maQuyen)){
        new MyDialog("Cập nhật quyền thành công!", MyDialog.SUCCESS_DIALOG);
    }else{
        new MyDialog("Cập nhật quyền thất bại!", MyDialog.ERROR_DIALOG);
    }
